...
- 「非公式ダッシュボード」「一覧」「確認」「推移」「チェーン別」「導入相談」など、用途が明確で公式誤認を避ける表現を優先する。
...
このドキュメントでは、現行サイトと制作物に必要な共通 UI / motion ルールを定義します。ロゴ利用規定、法務レビュー済みのコピー規定、広告テンプレート、SNS 投稿テンプレートは、提供元と非公式表記の運用要件が固まった段階で別章として追加します。
